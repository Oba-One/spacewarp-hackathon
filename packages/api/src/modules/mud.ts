...
import {
  Component as SolecsComponent,
  World as WorldContract,
} from "@latticexyz/solecs";
...
export async function setupContracts<C extends ContractComponents>(
  ...
  const signerOrProvider = computed(
    () => network.signer.get() || network.providers.get().json
  );

  const { contracts, config: contractsConfig } = await createContracts<{
    World: WorldContract;
  }>({
  ...
  const { txQueue, dispose: disposeTxQueue } = createTxQueue(
    contracts,
    network
  );
...
async function createEncoders(
  ...
  async function fetchAndCreateEncoder(entity: EntityIndex) {
    ...
    const [componentSchemaPropNames, componentSchemaTypes] =
      await componentContract.getSchema();
    encoders[componentId] = createEncoder(
      componentSchemaPropNames,
      componentSchemaTypes
    );
  }

  // Initial setup
  for (const entity of getComponentEntities(components))
    fetchAndCreateEncoder(entity);
...
function applyNetworkUpdates<C extends Components>(
  ...
  const ecsEventSub = ecsEvent$.subscribe((update) => {
    const entityIndex =
      world.entityToIndex.get(update.entity) ??
    ...
    if (update.value === undefined) {
      ...
      removeComponent(
        components[componentKey] as Component<Schema>,
        entityIndex
      );
    } else {
      setComponent(
