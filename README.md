...
- Game can be joined by using the game id and accepting the wager, and creating a pool.
...
2. Upon joining the game the user has a randomly generated deck of 12 characters.
3. The game is played in 6 turns, each turn the user can play cards based on their energy level an the card, with the goal of winning 2 or more locations.
4. The game is over after the 6th turn and the winner determined by the number of locations they have won.
...
*Note*: If doing Unity development you can run the `yarn build:unity` command to build the unity game and copy the build to the web client.
